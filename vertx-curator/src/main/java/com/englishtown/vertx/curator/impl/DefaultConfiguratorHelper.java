...
import com.englishtown.vertx.curator.*;
...
import static com.englishtown.vertx.curator.MatchBehavior.FIRST;

/**
 */
public class DefaultConfiguratorHelper implements ConfiguratorHelper {

    private final CuratorConfigurator configurator;
    private final CuratorClient curatorClient;
    private final CuratorOperationBuilders curatorOperationBuilders;
    ...
    public DefaultConfiguratorHelper(
        ...
        this.curatorOperationBuilders = curatorOperationBuilders;
        this.vertx = vertx;

        configurator.onReady(result -> init(configurator));
    }

    private void init(CuratorConfigurator configurator) {
    ...
    public void getConfigElement(String elementPath, CuratorWatcher watcher, MatchBehavior matchBehavior, Handler<AsyncResult<ConfigElement>> callback) {
        ...
        for (int i = 0; i < pathSuffixes.size(); i++) {
            ...
            CuratorOperation operation = curatorOperationBuilders.getData()
                    .forPath(path)
                    .build();
                    ...
                    case OK:
                        if (nodeMatches(event, matchBehavior)) {
                            ...
                            CuratorOperation operation = curatorOperationBuilders.getData()
                                    ...
                                    .build();
                            curatorClient.execute(operation, result2 -> {
    ...
    public ConfiguratorHelper usingNamespace(String namespace) {
        return new DefaultConfiguratorHelper(
                configurator,
                curatorClient.usingNamespace(namespace),
                curatorOperationBuilders,
                vertx);
    }
